...
def Memory_evaluate(memorys, test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256,
            is_visual=False, mask_labels_path=None, save_path=None, labels_dict=None): 
    #init
    psnr_list = {}
    roi_psnr_list = {}
    ...
    for k, (imgs, bboxes, flow) in enumerate(tqdm(test_dataloader, desc='test', leave=False)):
        ...
        with torch.no_grad():
            for i in range(object_num):
                output, _, _, memorys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss \
                    = generator(inputs[:, i], bboxes[i], memorys)
