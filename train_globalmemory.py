...
import utils
from models.preAE import PreAEMemory
...
from evaluate_globalmemory import *
    ...
    for epoch in range(config['epochs']):
        ...
        for j, (imgs, bboxes, flow) in enumerate(tqdm(train_dataloader, desc='train', leave=False)):
            ...
            for k in range(object_num):
                output, new_items = model(inputs[:, k], bboxes[k], items)
                items = new_items.detach()
                target = targets[:, k]
                if config['use_D']:
        ...
        if epoch % save_epoch == 0 or epoch == config['epochs'] - 1:
            ...
            # TODO 
            frame_AUC = ObjectLoss_evaluate(items, test_dataloader, model, labels_list, videos, dataset=config['test_dataset_type'],device = device,
